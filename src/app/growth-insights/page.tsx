...
export default function GrowthInsightsPage() {
  ...
  async function refresh(){try{const g=await apiFetch<Growth>("/api/v1/recommendations/growth",{method:"POST"});setBest(g.best_time);setRecs(g.recommendations.length?g.recommendations:fallback);setLive(true);}catch{setLive(false);}}
  const score=Math.max(70,Math.min(96,Math.round((best.confidence||.8)*100)));
  return (
    <AppShell title="Growth Insights" subtitle="AI-powered recommendations for audience growth" actions={<button onClick={refresh} className="primary-btn !min-h-9 !px-4 !py-2 text-xs">✦ Refresh insights</button>}>
