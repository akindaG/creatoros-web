...
export default function SocialAccountsPage() {
  ...
  useEffect(()=>{const controller=new AbortController();apiFetch<Account[]>("/api/v1/social-accounts",{signal:controller.signal}).then(data=>{setAccounts(data);setLive(true);}).catch(()=>{});return()=>controller.abort();},[]);
  function isConnected(platform:Platform){return accounts.some(a=>a.platform.toLowerCase()===platform.toLowerCase()&&a.status.toLowerCase()==="connected");}
  async function toggle(platform: Platform) {
    setError("");
    setPending(platform);
    try {
      const existing=accounts.find(a=>a.platform.toLowerCase()===platform.toLowerCase());
      if(existing){if(!window.confirm(`Disconnect ${platform} from CreatorOS?`))return;await apiFetch(`/api/v1/social-accounts/${existing.id}`,{method:"DELETE"});setAccounts(a=>a.filter(x=>x.id!==existing.id));return;}
