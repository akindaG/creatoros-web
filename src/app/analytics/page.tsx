...
export default function AnalyticsPage() {
  ...
  useEffect(()=>{const controller=new AbortController();apiFetch<Analytics>("/api/v1/analytics/dashboard",{signal:controller.signal}).then(v=>{setData(v);setLive(true);}).catch(()=>{});return()=>controller.abort();},[]);
  async function exportReport(){setExporting(true);setExportError("");try{const token=getToken();const r=await fetch(`${API_BASE}/api/v1/analytics/report`,{headers:token?{Authorization:`Bearer ${token}`}:{}});if(!r.ok)throw new Error(`Export failed (${r.status})`);const blob=await r.blob();const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="creatoros-analytics.csv";document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),0);}catch(error){setExportError(error instanceof Error?error.message:"Could not export analytics");}finally{setExporting(false);}}
  return (
    <AppShell title="Analytics Overview" subtitle="Deep insights into your performance" actions={<button onClick={exportReport} disabled={exporting} className="secondary-btn !min-h-9">{exporting?"Exporting...":"Export CSV"}</button>}>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div className="pill border-violet-300/30 bg-violet-500/10 text-violet-100">Overview</div><span className={`pill ${live?"text-[#4edea3]":"text-[#ffb95f]"}`}><span className={live?"status-dot":"h-2 w-2 rounded-full bg-[#ffb95f]"}/>{live?"Live backend data":"Demo fallback"}</span></div>
      {exportError&&<div role="alert" className="mb-4 rounded-xl border border-rose-300/10 bg-rose-400/[.06] p-3 text-xs text-rose-200">{exportError}</div>}
